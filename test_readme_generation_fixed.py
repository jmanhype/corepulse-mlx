...
# Generate with research-backed settings (generate_latents returns a generator)
print("   Generating with CFG 12.0 (not 7.5\!)...")
latents_generator = sd.generate_latents(
    prompt,
...
# Get the final latents from generator
for step, (latents_step, _) in enumerate(latents_generator):
    latents = latents_step
    if step == 0:
        print(f"   ✅ Generation started, latents shape: {latents.shape}")

print(f"   ✅ Final latents shape: {latents.shape}")
print(f"   ✅ CFG 12.0 applied successfully (20 steps completed)")
...
print("   (README example simplified - actual code uses generator)")
